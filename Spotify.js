...
async function GetSongs(folder) {
  ...
  // We use "info.json", as GitHub Pages doesn't support directory listing
  try {
...
const playMusic = (track, pause = false) => {
  ...
  document.querySelector(".songinfo-bar").innerText = track;
  document.querySelector(".songtime").innerHTML = "00:00 / 00:00";
};

async function DisplayAlbums() {
  try {
    let folders = ["ncs", "AP_Dillhon", "TSN"]; // Manually list folders, since GitHub Pages can’t list them
